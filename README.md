...
* `min_replicas_available` (type: _integer_, allowed: any number, no default) — The minimum number of replicas that need to be available for a node to be classified as `partial` rather than `dead`, where one or more replicas are `dead`. If not specified, any `dead` replica will cause a service to be marked `dead`
