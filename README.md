...
_Tested at Rust version: `rustc 1.61.0 (fe5b13d68 2022-05-18)`_
...
_Each release binary comes with an `.asc` signature file, which can be verified using the GPG public key: [:key:valeriansaliou.gpg.pub.asc](https://valeriansaliou.name/files/keys/valeriansaliou.gpg.pub.asc)._
...
```bash
docker pull valeriansaliou/vigil:v1.23.1
```
...
```bash
docker run -p 8080:8080 -v /path/to/your/vigil/config.cfg:/etc/vigil.cfg valeriansaliou/vigil:v1.23.1
```
